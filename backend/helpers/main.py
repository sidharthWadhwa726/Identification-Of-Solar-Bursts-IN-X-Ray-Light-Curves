...
from .post.classify import classify_catalog  # <- uses your trained scaler.pkl + kmeans_model.pkl
...
def build_argparser():
    ...
    # classification artifacts (scaler.pkl + kmeans_model.pkl)
    ap.add_argument("--cl-artifacts", type=str, default="cl_artifacts",
                    help="Folder containing scaler.pkl and kmeans_model.pkl (default: cl_artifacts)")
    return ap
...
def main():
    ...
    # Base catalog going forward (no AE anywhere)
    ...
    # -------------- CLASSIFICATION (KMeans artifacts) --------------
    try:
        base = classify_catalog(base, artifacts_dir=args.cl_artifacts)
        if "Solar_Class" in base.columns:
            print("[CL] Added Solar_Class via clustering artifacts.")
        else:
            # keep schema stable
            base["Solar_Class"] = np.nan
            print("[CL] Classification skipped; Solar_Class filled with NaN.")
    except Exception as e:
        print(f"[CL] Classification skipped due to error: {e}")
        if "Solar_Class" not in base.columns:
            base["Solar_Class"] = np.nan

    # -------------- WRITE FINAL CSV --------------
    base.to_csv("final_flare_catalog.csv", index=False)
    print(f"[OK] wrote final_flare_catalog.csv ({len(base)} rows) with Solar_Class column")
